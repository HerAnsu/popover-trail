...
> {
  ...
  action: (payload: TPayload, parentData?: TParentData, context?: TContext) => Promise<TResult> | TResult;
...
export const appSchema = createPopoverSchema({
  ...
  editUserStatus: {
    resolver: async (key, parentData: { id: string }) => fetch(`/api/users/${parentData.id}/status`).then((r) => r.json()),
    mutations: {
      updateStatus: {
