...
export default {
  meta: {
    ...
    docs: {
      description:
        'Enforce environment check for window.crypto / globalThis.crypto before subtle crypto access.',
      category: 'Security',
      recommended: true,
    },
    schema: [],
    messages: {
      requireCryptoGuard:
        'Access to crypto.subtle in {{ name }} must be guarded with typeof crypto !== "undefined".',
    },
  },
  create(context) {
        ...
        if (
          ...
          if (parent) {
            ...
            if (
              ...
              !body.includes('globalThis.crypto')
            ) {
              context.report({
                node,
