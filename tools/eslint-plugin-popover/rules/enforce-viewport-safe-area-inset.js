...
export default {
  meta: {
    ...
    schema: [],
    messages: {
      suggestSafeArea: 'Viewport boundary calculation in {{ name }} should account for safe-area-inset.',
    },
  },
  create(context) {
    ...
    return {
      FunctionDeclaration(node) {
        if (node.id && (node.id.name.includes('EdgeInsets') || node.id.name.includes('FloatingBoundary'))) {
          const body = context.getSourceCode ? context.getSourceCode().getText(node) : '';
          if (!body.includes('safeArea') && !body.includes('safe-area') && !body.includes('padding')) {
            context.report({
              node,
