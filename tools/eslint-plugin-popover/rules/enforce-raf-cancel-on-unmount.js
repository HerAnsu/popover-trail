...
export default {
  meta: {
    ...
    schema: [],
    messages: {
      requireRafCancel: 'Effect schedules requestAnimationFrame but does not call cancelAnimationFrame in cleanup function.',
    },
  },
  create(context) {
    ...
    return {
      CallExpression(node) {
        if (
          ...
          const body = context.getSourceCode ? context.getSourceCode().getText(node.arguments[0]) : '';
