...
export default {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Enforce pure mathematical functions without hidden DOM or global state dependencies.',
      category: 'Clean Code',
      recommended: true,
    },
    schema: [],
    messages: {
      impureMathFunction: 'Math function {{ name }} should be pure; avoid referencing global {{ globalName }}.',
    },
  },
  create(context) {
    ...
    return {
      FunctionDeclaration(node) {
        if (node.id && (node.id.name.startsWith('compute') || node.id.name.startsWith('calculate'))) {
