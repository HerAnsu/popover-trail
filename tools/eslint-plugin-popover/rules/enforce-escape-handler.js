...
module.exports = {
  meta: {
    ...
    schema: [],
    messages: {
      missingEscape: 'Card keyboard navigation handler should support dismissal via the `Escape` key.',
    },
  },
  create(context) {
    return {
      SwitchStatement(node) {
        if (node.discriminant && node.discriminant.property && node.discriminant.property.name === 'key') {
