...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Disallow Math.random() in token generators; use crypto.getRandomValues or crypto.randomUUID.',
      category: 'Security',
      recommended: true,
    },
    schema: [],
    messages: {
      noMathRandomForTokens: 'Do not use Math.random() for security token generation in {{ name }}; use crypto.getRandomValues().',
    },
  },
  create(context) {
