...
export default {
  meta: {
    ...
    docs: {
      description:
        'Ensure forwardRef and memo components define an explicit displayName for React DevTools',
    ...
    messages: {
      missingDisplayName:
        'Component wrapped in `{{wrapper}}` should declare an explicit `displayName`.',
    },
  },
  create(context) {
    return {
        ...
        if (
          ...
          if (!src.includes(`${compName}.displayName`) && compName !== 'Component') {
            context.report({
              ...
              data: { wrapper: node.init.callee.name },
            });
          }
        }
      },
