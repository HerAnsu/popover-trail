...
export function isBufferDomainError(val: unknown): val is BufferDomainError {
  ...
  const c = val.code;
  return c === 'INVALID_CAPACITY' || c === 'BUFFER_OVERFLOW' || c === 'BUFFER_EMPTY' || c === 'INDEX_OUT_OF_BOUNDS';
}
export function isInvalidCapacityError(val: unknown): val is InvalidCapacityError {
  return typeof val === 'object' && val !== null && 'code' in val && val.code === 'INVALID_CAPACITY';
}
...
export function isIndexOutOfBoundsError(val: unknown): val is IndexOutOfBoundsError {
  return typeof val === 'object' && val !== null && 'code' in val && val.code === 'INDEX_OUT_OF_BOUNDS';
}
...
export function matchBufferError<R>(
  ...
  switch (err.code) {
    case 'INVALID_CAPACITY': return cases.INVALID_CAPACITY(err);
    case 'BUFFER_OVERFLOW': return cases.BUFFER_OVERFLOW(err);
    case 'BUFFER_EMPTY': return cases.BUFFER_EMPTY(err);
    case 'INDEX_OUT_OF_BOUNDS': return cases.INDEX_OUT_OF_BOUNDS(err);
...
export function createInvalidCapacityError(cap: unknown, msg?: string): InvalidCapacityError {
  ...
  return { code: 'INVALID_CAPACITY', message, capacity: typeof cap === 'number' ? cap : Number.NaN };
}
export function createBufferOverflowError(cap: number, msg?: string): BufferOverflowError {
  return { code: 'BUFFER_OVERFLOW', message: msg ?? `RingBuffer capacity limit (${cap}) exceeded`, capacity: cap };
}
...
export function createIndexOutOfBoundsError(idx: number, size: number, msg?: string): IndexOutOfBoundsError {
  return { code: 'INDEX_OUT_OF_BOUNDS', message: msg ?? `Index ${idx} is out of bounds for buffer size ${size}`, index: idx, size };
}
