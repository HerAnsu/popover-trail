...
import { createRingBufferSafe, createRingBufferFromSafe, type BufferDomainError } from './bufferResult';
import { isArray } from './bufferGuards';
import { RingBufferDeque } from './ringBufferDeque';
import type { BufferCapacity } from './bufferBranded';
import type {
  RingBufferOptions, RingBufferMetrics, RingBufferState, ReadonlyRingBuffer,
  BufferPredicate, BufferTypeGuard, BufferTransform,
} from './bufferTypes';

export class RingBuffer<T> extends RingBufferDeque<T> implements ReadonlyRingBuffer<T>, ScopeDisposable {
  override readonly state: RingBufferState<T>;

  static create<T>(opt: BufferCapacity | number | RingBufferOptions<T>, evict?: (i: T) => void): Result<RingBuffer<T>, BufferDomainError> {
    return createRingBufferSafe(opt, evict, (o, e) => new RingBuffer(o, e));
  }
  static from<T>(items: Iterable<T>, cap?: BufferCapacity | number): Result<RingBuffer<T>, BufferDomainError> {
    return createRingBufferFromSafe(items, cap, (o, e) => new RingBuffer(o, e));
  }

  constructor(options: BufferCapacity | number | RingBufferOptions<T>, onEvict?: (item: T) => void) {
    super();
  ...
  clone(): RingBuffer<T> { return cloneRing(this.state, (opt) => new RingBuffer<T>(opt)); }
  resize(newCapacity: BufferCapacity | number): void { resizeRing(this.state, newCapacity); }
  shrinkToFit(): void { shrinkRingToFit(this.state); }
