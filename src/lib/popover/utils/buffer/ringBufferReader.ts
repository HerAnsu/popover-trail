...
import type { BufferCapacity, BufferRevision, BufferLogicalIndex, BufferRelativeIndex } from './bufferBranded';
...
export abstract class RingBufferReader<T> extends RingBufferQuery<T> {
  ...
  toArray(): T[] { return toArrayRing(this.state); }
  toReversedArray(): T[] { return toReversedArrayRing(this.state); }
