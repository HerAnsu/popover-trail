...
export abstract class RingBufferDeque<T> extends RingBufferReader<T> {
  ...
  push(item: T): void { pushItem(this.state, this.metrics, item, (cap) => this.resize(cap)); }
  ...
  pop(): T | undefined { return popItem(this.state, this.metrics); }
  popResult(): Result<T, BufferEmptyError> { return popRingResult(this.state, this.metrics); }
  shift(): T | undefined { return shiftItem(this.state, this.metrics); }
  shiftResult(): Result<T, BufferEmptyError> { return shiftRingResult(this.state, this.metrics); }
  unshift(item: T): void { unshiftItem(this.state, this.metrics, item, (cap) => this.resize(cap)); }

  swap(indexA: BufferRelativeIndex, indexB: BufferRelativeIndex): boolean {
    return swapRingItems(this.state, indexA, indexB);
  }
  reverse(): void { reverseRing(this.state); }
  fill(value: T): void { fillRing(this.state, value); }
  clear(): void { clearRingBufferState(this.state); }
