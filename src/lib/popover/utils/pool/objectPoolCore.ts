...
export class ObjectPool<T> extends ObjectPoolBase<T> {
  ...
  acquireMany(count: number, out: T[] = []): T[] {
    return acquireManyItems(() => this.acquire(), count, out);
  }
  releaseMany(items: Iterable<T | null | undefined>): void {
    releaseManyItems((i) => this.release(i), items);
  }
  acquireWith(init: (item: T) => void): T {
  ...
  runWithInit<R>(init: (i: T) => void, fn: (i: T) => R): R {
    return runWithItem(
      () => this.acquireWith(init),
      (i) => this.release(i),
      fn,
    );
  }
  runWithAsync<R>(fn: (item: T) => Promise<R>): Promise<R> {
    return runWithItemAsync(
      () => this.acquire(),
      (i) => this.release(i),
      fn,
    );
  ...
  useResult<R, E>(fn: (item: T) => Result<R, E>): Result<R, E> {
    return runWithItemResult(
      () => this.acquire(),
      (i) => this.release(i),
      fn,
    );
  }
  useAsync<R>(fn: (item: T) => Promise<R>): Promise<R> {
    return this.runWithAsync(fn);
  }
  usePair<R>(fn: (a: T, b: T) => R): R {
    return runWithPair(
      () => this.acquire(),
      (i) => this.release(i),
      fn,
  ...
  useTriple<R>(fn: (a: T, b: T, c: T) => R): R {
    return runWithTriple(
      () => this.acquire(),
      (i) => this.release(i),
      fn,
    );
  }
  usePairAsync<R>(fn: (a: T, b: T) => Promise<R>): Promise<R> {
    return runWithPairAsync(
      () => this.acquire(),
      (i) => this.release(i),
      fn,
    );
  ...
  scope(): ScopedPooledItem<T> {
    return this.acquireScoped();
  }
  acquireScoped(): ScopedPooledItem<T> {
    return createScopedItem(
      () => this.acquire(),
      (i) => this.release(i),
    );
  }
