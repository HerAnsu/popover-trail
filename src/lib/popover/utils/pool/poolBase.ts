...
export abstract class ObjectPoolBase<T> {
  ...
  constructor(cfg: ObjectPoolResolvedConfig<T>) {
    ...
    this.adaptive = new AdaptiveDrainController(() => this.drain(cfg.safeInitial), cfg.idleDrainTimeoutMs);
  }

  addObserver(observer: PoolObserver<T>): () => void { return this.observers.subscribe(observer); }
  acquire(): T {
  ...
  release(item?: T | null): void {
    if (executeRelease(this.storage, item, this.reset, this.onEvict, this.tracker, this.sentinel, this.observers)) {
  ...
  shrinkToFit(minCap = 0): number { return shrinkPoolToFit(this.storage, minCap, this.onEvict); }
  drain(keep = 0): void {
    const drained = this.storage.drain(keep);
    for (const item of drained) { tryEvictItem(this.onEvict, item); this.observers.notifyEvict(item); }
    this.observers.notifyDrain(drained.length);
  }
  clear(): void { this.sentinel.clear(); this.drain(0); }
  dispose(): void { this.isDisposed = true; this.adaptive.dispose(); this.clear(); }
  resetItem(item: T): void { tryResetItem(this.reset, item); }
  getHealth(): PoolHealthReport { return assessPoolHealth(this.getMetrics(), this.sentinel.getLeakedItems().size); }
  inspect(): string {
  ...
  get capacity(): number { return this.storage.capacity; }
  get inUse(): number { return Math.max(0, this.tracker.acquired - this.tracker.released); }
  getMetrics(): ObjectPoolMetrics { return this.tracker.getSnapshot(this.storage.size, this.storage.capacity); }
  getLeakedObjects(tMs?: number, now?: number): Map<T, LeakedItemInfo> { return this.sentinel.getLeakedItems(tMs, now); }
}
