...
export class KeyedPool<K, T> {
  ...
  getPool(key: K): ObjectPool<T> {
    ...
    if (!pool) {
      const opts = typeof this.optionsProvider === 'function'
  ...
  acquire(key: K): T { return this.getPool(key).acquire(); }
  release(key: K, item?: T | null): void { this.pools.get(key)?.release(item); }
  runWith<R>(key: K, fn: (item: T) => R): R { return this.getPool(key).runWith(fn); }
  use<R>(key: K, fn: (item: T) => R): R { return this.runWith(key, fn); }
  hasPool(key: K): boolean { return this.pools.has(key); }
  get partitionCount(): number { return this.pools.size; }
  getMetrics(key: K): ObjectPoolMetrics | undefined { return this.pools.get(key)?.getMetrics(); }
  ...
  drain(key: K, keep = 0): void { this.pools.get(key)?.drain(keep); }
  drainAll(keep = 0): void { for (const pool of this.pools.values()) pool.drain(keep); }
  clearAll(): void { for (const pool of this.pools.values()) pool.clear(); }
  dispose(): void { for (const pool of this.pools.values()) pool.dispose(); this.pools.clear(); }
...
export function createBucketPool<T>(
  ...
  const pool = new KeyedPool<number, T>((size) => factory(size), { factory: () => factory(0), reset });
