...
describe('viewportGeometry', () => {
  ...
  describe('resolveMiddlewareProps', () => {
    ...
    it('returns empty object for primitives, arrays, null, or undefined', () => {
      expect(resolveMiddlewareProps(null)).toEqual({});
      expect(resolveMiddlewareProps(undefined)).toEqual({});
      expect(resolveMiddlewareProps('string')).toEqual({});
      expect(resolveMiddlewareProps(42)).toEqual({});
    });
  });

  describe('resolveAutoPlacement', () => {
    it('preserves non-auto placement unmodified', () => {
      expect(resolveAutoPlacement('top-start', null)).toBe('top-start');
      expect(resolveAutoPlacement('bottom', undefined)).toBe('bottom');
    });

    it('falls back to "right" when anchorRect is null or undefined', () => {
      expect(resolveAutoPlacement('auto', null)).toBe('right');
      expect(resolveAutoPlacement('auto', undefined)).toBe('right');
    });
    ...
    it('resolves auto placement to left or right based on viewport center', () => {
      ...
      expect(resolveAutoPlacement('auto', rightAnchor)).toBe('left');

      // Anchor on left half: center at 250 <= 500 -> right
      const leftAnchor = { left: 200, width: 100, top: 0, height: 50, bottom: 50, right: 300 };
      expect(resolveAutoPlacement('auto', leftAnchor)).toBe('right');
    });
  });

  describe('resolveResponsivePosition', () => {
    it('calculates bottom-sheet coordinates on mobile viewport', () => {
      const pos = resolveResponsivePosition('bottom-sheet', true, undefined, 500, 800);
      expect(pos).toEqual({ top: 480, left: 50 });
    });

    it('calculates centered modal coordinates', () => {
      const pos = resolveResponsivePosition('modal', false, undefined, 1000, 750);
      expect(pos).toEqual({ top: 200, left: 300 });
    });

    it('calculates docked-top coordinates', () => {
      const pos = resolveResponsivePosition(undefined, false, 'docked-top', 1000, 700);
      expect(pos).toEqual({ top: 10, left: 300 });
    });

    it('returns null for default non-responsive standard layout', () => {
      const pos = resolveResponsivePosition(undefined, false, undefined, 1000, 800);
      expect(pos).toBeNull();
    });
  });
});
